...
def check_fraud_density_quality(
    ...
    try:
        ...
        null_market = [
            i for i, r in enumerate(rows) if not str(r.get("market_type", "")).strip()
        ]
        if null_market:
            errors.append(
                f"market_type has {len(null_market)} null/empty value(s) at row indices: {null_market[:5]}"
            )
        ...
        zero_tx = [
            r["market_type"]
            for r in rows
            if (to_int(r.get("transaction_count")) or 0) <= 0
        ]
        if zero_tx:
        ...
        out_of_range = [
            r["market_type"]
            for r in rows
            if (v := to_float(r.get("fraud_rate_pct"))) is not None
            and not (0.0 <= v <= 100.0)
        ]
        if out_of_range:
            errors.append(
                f"fraud_rate_pct out of [0.0, 100.0] for categories: {out_of_range[:5]}"
            )
