    ...
    .schema(
        {
            ...
            "specialty": (
                string(),
                "Primary medical specialty of the account (e.g. Cardiology, Oncology, Primary Care).",
            ),
            "city": (string(), "City where the account is located."),
            "state": (string(), "Two-letter US state code where the account is located."),
            "prescribing_decile": (
                int64(),
                "Prescribing volume decile from 1 (lowest) to 10 (highest) relative to peers.",
            ),
            ...
            "potential_value_usd": (
                int64(),
                "Estimated total addressable revenue opportunity for the account, in USD.",
            ),
            "actual_value_usd": (int64(), "Realized revenue attributed to the account to date, in USD."),
            "preferred_channel": (
                string(),
                "Account's preferred engagement channel (e.g. F2F, Remote, Email, Phone, Conference).",
            ),
    ...
    .schema(
        {
            ...
            "account_id": (
                string(),
                "Identifier of the account the activity was logged against; joins to account.account_id.",
            ),
            "rep_id": (string(), "Identifier of the sales representative who performed the activity (e.g. 'R-001')."),
            "territory_id": (
                string(),
                "Identifier of the sales territory in which the activity occurred (e.g. 'T-01').",
            ),
            "activity_datetime": (timestamp(unit=DurationUnit.Nanoseconds), "Date and time the activity took place."),
            "activity_month": (
                string(),
                "Calendar month of the activity in 'YYYY-MM' format, derived from activity_datetime.",
            ),
            "channel": (
                string(),
                "Channel through which the activity was conducted (e.g. F2F, Remote, Email, Phone, Virtual Event).",
            ),
            "activity_type": (string(), "Type of engagement (e.g. Detail, Sample Drop, Follow-up, Medical Inquiry)."),
            "product_discussed": (
                string(),
                "Product or SKU discussed during the activity (e.g. 'Cardivex 10mg', 'Neurolyn').",
            ),
            "detail_priority": (
                int64(),
            ...
            "on_preferred_channel": (
                boolean(),
                "Whether the activity was conducted via the account's preferred channel.",
            ),
            "sample_dropped": (boolean(), "Whether product samples were dropped during the activity."),
            "sample_quantity": (
                int64(),
                "Quantity of product samples left with the account; 0 when no samples were dropped.",
            ),
            "estimated_cost_usd": (int64(), "Estimated cost of conducting the activity, in USD."),
            "next_best_action": (
                string(),
                "Recommended follow-up action for the account (e.g. 'Schedule follow-up detail', 'Send approved email').",
            ),
