...
def _retrieve_cash_flows(tickers: yf.Tickers) -> pa.Table:
    ...
    for symbol, ticker in tickers.tickers.items():
        ...
        if isinstance(cash, dict):
            raise TypeError(
                f"Cash flow returned as dict by yfinance for symbol - {symbol}"
            )
    ...
    cash_flows = pa.Table.from_pandas(
        pd.concat(cash_flows, ignore_index=True), schema=CASH_FLOWS_SCHEMA
    )
...
def _retrieve_balance_sheets(tickers: yf.Tickers) -> pa.Table:
    ...
    for symbol, ticker in tickers.tickers.items():
        ...
        if isinstance(balance, dict):
            raise TypeError(
                f"Balance sheet returned as dict by yfinance for symbol - {symbol}"
            )
    ...
    balance_sheets = pa.Table.from_pandas(
        pd.concat(balance_sheets, ignore_index=True), schema=BALANCE_SHEETS_SCHEMA
    )
...
def transform(
    ...
    parquet_to_adls(
        adls, balance_sheets, adls.model_paths[balance_sheets_model.name].path
    )
    parquet_to_adls(
        adls, source_documents, adls.model_paths[source_documents_model.name].path
    )
